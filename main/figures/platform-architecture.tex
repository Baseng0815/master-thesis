...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[alt]     (nat) at ( 2.7,-2.6) {\code{mogi\_cpu}: interpreter, JIT\\[2pt]
                                         \code{mogi\_mem}: \gls{mmu}, page tables\\[2pt]
  ...
  \caption[The MOGI platform]{%
    The MOGI platform.
    \code{mogi\_emulator} models a Linux user-space system including process
    scheduling, system call handling, a virtual filesystem, network sockets, and the clock and
    random sources.
    The platform is exposed in a trait \code{Emulator}
    that runs guest binaries until a budget is exhausted or the guest blocks.
    The execution backend is defined through traits and therefore generic.
    We use the Unicorn backend (\cref{sec:impl-platform}).
  }
