...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[net] (VP1) at ( 7.0,-2.5) {$VP$};
    \node[net] (VP2) at (10.6,-2.5) {$VP$};
  ...
  \caption[The EfficientZero architecture]{%
    ...
    The representation network $R$ encodes the observation history into the root
    latent state $s^0$ --- the only state in the diagram that corresponds to a
    real observation.
    The dynamics network $G$ advances a latent state by one action,
    $s^k = G(s^{k-1},a^k)$, so the unroll can be continued to any depth without
    touching the environment.
    Every latent state is scored by the policy head $P$ and the value head $V$.
    The value-prefix network $VP$ is recurrent: instead of a per-step reward it
    consumes the imagined latent states in sequence, carrying an LSTM state
    along the unroll, and predicts the reward accumulated since the root
    (\cref{sec:ez-value-prefix}).
    All five networks share the parameter vector $\theta$ and are trained
    jointly (\cref{sec:ez-loss}).
  }
