...
\begin{description}[style=nextline, leftmargin=0pt, labelindent=0pt, labelwidth=0pt]
  ...
  \item[Fang and Yan (2018)]
    LEFT, an emulation-instrumented fuzzer for the 4G/LTE stack of Android mobile
    devices guided by Q-learning --- the one surveyed work whose setting resembles
    ours, in that the target runs under an emulator rather than natively.
    The state is the protocol message type, the action is the mutation operator, and
    the reward combines emulator crashes, information leakage, and loss of network
    connectivity.
    \todo{Read the primary source (ESORICS 2018) once the bib entry exists --- the
    state/action/reward description here is the survey's~\cite{wang_systematic_2020}
    and is too coarse to be trusted. This paper is worth a subsection of its own
    given the shared emulation setting.}

  \item[Liu et al.\ (2019)~\cite{authors_reinforcement_nodate}]
    FuzzBoost, which transplants Böttinger's formulation from file parsers to
    compilers, with GCC as the environment and C programs as the inputs.
    \begin{itemize}
      \item \textbf{State.}
        As in Böttinger: substrings of the seed program, $I := \Sigma^{*}$ with the
        states drawn from $S(p)$, observed through a window of size $w$ at
        offset~$s$.
      \item \textbf{Action.}
        Again mappings from the observed substring to probabilistic rewrite rules,
        but split into two families.
        Content rewrites operate at token level and respect the C lexical grammar
        --- insertion, replacement, re-ordering, deletion, replication --- so that
        the mutant has a chance of surviving the front end.
        Window rewrites shift the window left or right and grow or shrink it by one
        character, leaving the program untouched.
        A third, explicit \emph{terminate} action lets the agent end the episode
        itself.
      \item \textbf{Reward.}
        A coverage \emph{ratio} rather than a count:
        $R(p, I') := |B(T_p)| \,/\, |\bigcup_{\rho \in I'} B(T_\rho)|$, the unique
        \glspl{bb} on the execution trace $T_p$ of the generated program over all
        \glspl{bb} the test suite $I'$ has covered so far.
        This is a continuous scalar in $(0, 1]$ that reaches~$1$ when a single trace
        covers everything seen so far, and it is discounted by $\gamma$ over the
        episode.
        Traces come from Pin.
    \end{itemize}
    The episodic framing is the real departure from Böttinger, whose loop never
    terminates: here an episode is the mutation of one seed program and ends either
    at a hard-coded trace length or when the agent takes the terminate action.

  \item[Kuznetsov et al.\ (2019)~\cite{kuznetsov_automated_2019}]
    The one entry the survey files under exploitability analysis rather than
    mutation operator selection, and the only one that observes the program instead
    of the input.
    \begin{itemize}
      \item \textbf{State.}
        The program's response, gathered black-box through a debugger: execution
        time, code coverage, code completion and returned error codes, preprocessed
        into the input layer of the $Q$~network.
        The agent never sees the test case it is mutating.
      \item \textbf{Action.}
        One of \num{45} mutation functions kept in a dictionary --- lengthening and
        shortening strings, inserting integers, appending format specifiers such as
        \code{\%s} --- one output unit of the network each.
      \item \textbf{Reward.}
        Granted when the execution time exceeds that of the previous step or when an
        error is observed; an error also terminates the episode.
        The authors report a reward-hacking failure here that is worth recording:
        having found one error, the agent learned to trigger it over and over to
        farm the reward.
        They counter it with the discount rate ($\gamma = 0.9$) and a high
        exploration factor ($\varepsilon = 0.5$, decayed by \num{0.99} per epoch)
        rather than by changing the reward.
    \end{itemize}
    The evaluation is a microbenchmark, not real software: \num{90} training and
    \num{10} validation targets, each a chain of \code{if}--\code{else} blocks
    arranged so that \numrange{20}{30} specific mutations in a row are needed to
    reach the error.
    On these, the agent needs about \SI{30}{\percent} fewer mutations than random
    selection.
...
abstraction of the protocol \cite{hutchison_autonomic_2010} or of the program's
response \cite{kuznetsov_automated_2019}.
The actions are, with hardly any exception, the mutation operators the underlying
fuzzer already had; the learning replaces the uniform random choice among them and
nothing else.
...
error indicators \cite{hutchison_autonomic_2010}.
