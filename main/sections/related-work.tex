...
Fuzzing is a sequential decision-making problem under uncertainty over a large search space
...
They further group the fuzzers by the learning algorithm each uses.
These are broadly divided into traditional machine learning, deep learning, and reinforcement learning.
The reinforcement-learning-based fuzzers that they survey for each fuzzing step are shown in \cref{tab:rw-rl-fuzzers}.

\input{figures/rw-rl-fuzzers}

\section{Modeling Fuzzing as a Markov Decision Process}\label{sec:rw-mdp-formulations}

For six of the seven papers that use a form of Q-learning or \gls{sarsa}, we
outline how they model their problem as an \gls{mdp} and
...
\begin{description}[style=nextline, leftmargin=0pt, labelindent=0pt, labelwidth=0pt]
  \item[Becker et al.\ (2010)~\cite{hutchison_autonomic_2010}]
    ...
    \begin{itemize}
      \item \textbf{State.}
        ...
        session (insert, repeat, or drop a message according to the behavioral
        model).
      ...
      \item \textbf{Reward.}
        ...
        where $r_{\text{trace}}$ depends on the entropy of a message and the
        number of function calls, $r_{\text{deb}}$ depends on whether
        an error was seen, and $r_{\text{mon}}$ depends on whether a corrupted
        or delayed message was monitored.
    \end{itemize}
    The paper provides only a design; there is no implementation and therefore
  ...
  \item[Böttinger et al.\ (2018)~\cite{bottinger_deep_2018}]
    ...
    \begin{itemize}
      ...
      \item \textbf{Action.}
        Actions are random variables mapping substrings of an input to probabilistic rewriting rules.
        The implemented rules include random bit flips, dictionary token insertion,
      ...
      \item \textbf{Reward.}
        ...
        where $E$ depends on runtime properties of the generated input $x$ and $G$
    ...
    \end{itemize}
    They find that neither experience replay nor a persistent history of discovered \glspl{bb}
    improves on a random baseline.

  \item[Drozd and Wagner (2018)~\cite{drozd_fuzzergym_2018}]
    FuzzerGym\index{FuzzerGym} combines libFuzzer with OpenAI Gym and trains its agent with deep double
    ...
    \begin{itemize}
      ...
      \item \textbf{Reward.}
        Reward is modeled as newly covered unique lines:
        \[
    ...
    \end{itemize}
    To maintain libFuzzer's peak rate of around \num{100000} executions per
  ...
  \item[Fang and Yan (2018)~\cite{lopez_emulation-instrumented_2018}]
    LEFT (\glsentryshort{lte}-Oriented Emulation-Instrumented Fuzzing Testbed)\index{LEFT} is a fuzzer for the
    4G/\gls{lte} protocol stack on Android devices, driven through a network emulator.

    \begin{itemize}
      \item \textbf{State.}
        States are modeled as message types on which the protocol implementation switches.
  ...
  \item[Liu et al.\ (2019)~\cite{liu_reinforcement_2019}]
    They introduce FuzzBoost\index{FuzzBoost}, a compiler fuzzer and automatic code synthesis framework.

    \begin{itemize}
      \item \textbf{State.}
        As in \textcite{bottinger_deep_2018}, states are program inputs
      ...
      \item \textbf{Reward.}
        ...
        \[
          R(p,I)=\frac{\lvert B(T_p)\rvert}{\left\lvert\bigcup_{\rho\in I} B(T_\rho)\right\rvert}
        \]
    \end{itemize}
    Unlike \textcite{bottinger_deep_2018}, who fuzz continuously, they use an episodic framing.

  \item[Kuznetsov et al.\ (2019)~\cite{kuznetsov_automated_2019}]
    They provide a general architecture for applying deep \gls{rl} to automated vulnerability
    testing.

    \begin{itemize}
      \item \textbf{State.}
        The authors do not describe a specific state model, but stay general and
      ...
      \item \textbf{Reward.}
        A reward is granted when the execution time of the mutated seed exceeds that
        of the previous seed, or when a crash is encountered.
...
\begin{itemize}
  \item \textbf{Better deep \gls{rl} algorithms.}
    While the line of \gls{dqn} has incorporated many improvements along the
    ...
    architecture. We propose using EfficientZero~\cite{ye_mastering_2021}
    as the core \gls{rl} algorithm for improved learning and sampling efficiency.
  ...
  \item \textbf{Coverage as the state model.}
    ...
    We propose to include coverage information in the observations that the \gls{rl} agent receives.
  ...
  \item \textbf{Generic reward signal.}
    A big advantage of \gls{rl}-based approaches is that they do not rely on binary decisions,
