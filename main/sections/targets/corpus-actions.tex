...
cJSON 1.7.19 is a single-file recursive-descent \gls{json} parser written in C~\cite{gamble_cjson_nodate}.\index{target!cJSON}
...
a buffer and a length and tries to parse the buffer content as \gls{json}.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}ll>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    ...
    \code{REUSE4}
      ...
      & 32 (reuse of $4\times$) \\
    \code{SMALL}/\code{FULL}
      & hidden width & latent width 128 & latent width 64/256 \\
    \code{CTRL-RAND}
...
The corpus holds a single 16-byte seed, an episode is bounded at 64 mutations and therefore 128 \gls{mdp}
...
apply mutations to reach valid \gls{json} syntax on its own.
...
libxml2 v2.15.3 is an \gls{xml} parser commonly used in Linux~\cite{veillard_libxml2_nodate}.\index{target!libxml2}
...
It is structured almost identically to~\cref{lst:target-cjson}.
...
picohttpparser is a header-only \gls{http}/1.x request parser~\cite{oku_picohttpparser_nodate}.\index{target!picohttpparser}
We use the version from the master branch with commit hash \code{f4d94b4}.
The setup is identical to libxml2 except for the block count of \num{4096}, since this is a smaller target.
