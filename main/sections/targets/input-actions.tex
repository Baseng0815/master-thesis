...
while \emph{sequence} and \emph{open62541} require the learner to solve a full \gls{pomdp} problem.
...
The reward is the raw sum of block hit counts.
...
As described in~\cref{sec:input-level-actions}, we model actions as actual \gls{sut} input
and a single environment step then consists of supplying a single byte to the \gls{sut}.
...
two exceptions.
First, since we are now in a full \gls{pomdp} setting instead of just a bandit, we include
...
open62541 is an open-source \gls{opcua} implementation~\cite{open62541_open62541_nodate}.\index{target!open62541}
We use the \code{tutorial\_server\_firststeps} example server as the harness
with the goal of learning an action sequence that reaches the connected state.

The action space is made up of 15 protocol messages instead of single bytes.
To reach the connect state, the agent must send \code{hel}, then \code{opn},
then \code{create\_sess} and finally \code{activate\_sess}.
We boot the server until it blocks on accepting a connection, inject a client connection and let the
server accept it, and then take a snapshot, so all episodes start with an existing TCP connection.
...
Coverage is part of the observation, but not the reward.
The episode ends when the server closes the channel after a rejected message,
when a session is activated, or after twelve steps.
The optimal policy in this case consists of looping for all 12 steps, with the
final step being the session activation that also terminates the episode.
