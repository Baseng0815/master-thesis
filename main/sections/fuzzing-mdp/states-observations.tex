...
For a given \gls{sut}, we model the state space as the joint state of process memory,
kernel state, the register file and any other execution state needed to run the \gls{sut}.
This constitutes a full description of the \gls{sut} and therefore suffices to fully predict future
behavior of the \gls{sut} given new inputs.
Although this state space formulation is straightforward, it is too large to serve as a
base for a fuzzing implementation.
In practice, this state is contained in the execution environment, and although it
is accessible to us, we do not use it directly.
Adhering to the approach of \gls{cgf}, the fuzzer instead observes coverage information $\mathcal{C}$.
In our case, this can be either block or edge coverage.
We therefore model the observation function $o_F:\mathcal{S}_F\rightarrow\mathcal{C}$ to map
true \gls{sut} state $\mathcal{S}_F$ to coverage feedback $\mathcal{C}$.
