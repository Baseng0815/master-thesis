...
Executing the \gls{put} produces a list of block hit counts from which the reward is built.
We do not use the hit counts directly, but rather map them to an \gls{afl}-style hit count \emph{level}
...
and a \emph{coverage score} is derived from the hit count levels by summing over all buckets:
...
Compressing the raw counts into exponentially spaced buckets make the coverage score less sensitive
to the exact hit count and allows to represent a wider range of hit counts.

\subsection{Input-level actions}

In the input-level actions setting, the fuzzer executes single inputs and keep no execution history around.
...
each step it keeps the environment alive, as well as a final coverage reward
whenever it manages to reach its goal.
The high-and-low (\cref{sec:target-high-and-low}) example is awarded a raw coverage score.

\subsection{Corpus-level actions}

In general, rewards for corpus-level actions can either be \emph{memoryless}
or \emph{marginal}.
Memoryless rewards depend only on the coverage score of each execution and ignore
any coverage reached by previous seeds.
In contrast, marginal coverage pays only for the levels that have not been reached before.
...
regions.
A problem is mutations producing the same high-coverage seed over and over again.
Paying out the same high-coverage reward for seeds that already reach a high coverage
discourages exploration into new regions.

We solve this problem by introducing a marginal reward.
By keeping track of a union of all coverage seen during an episode, we can
subtract this union from a new coverage to get a marginal coverage.
...
A seed that achieves new coverage early in the run when the coverage union is still sparse
...
larger than any previous execution.

An episode maintains two running coverage maxima over its executions: the
\emph{union coverage} and the \emph{depth record}.
...
that encourates exploration depth by rewarding \emph{deep} executions
...
where $\omega$ is a parameter that adjusts the weight between exploration breadth
versus depth and $\lambda$ rescales the reward to better fit the value support.
For the default choice of $\omega=1$, we end up with a reward signal that values a
seed that reaches exactly one level deeper than the union of all previous seeds equally to a seed that 
reaches exactly one level deeper than the best previous seed.
