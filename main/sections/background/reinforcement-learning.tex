...
finite Markov decision processes (MDPs). Formally, a Markov decision process is
...
Some algorithms require that a model of the transition function $p$ be available.
...
\emph{Model-free methods} do not base their estimates on an underlying model, but
...
\paragraph{SARSA}
SARSA~\cite{sutton_reinforcement_2020} is similar to TD learning, but learns an action-value function $Q(s,a)$ instead of
