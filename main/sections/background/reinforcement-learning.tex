...
\Gls{rl}\index{reinforcement learning} is an extremely general framework for approaching problems
...
The problems reinforcement learning is typically applied to can be modeled as
finite \glspl{mdp}.\index{Markov decision process} Formally, a Markov decision process is
a 3-tuple $\mathit{MDP}=(\mathcal{S},\mathcal{A},p)$ where
...
\begin{itemize}
  ...
  \item $p(s^\prime,r|s,a)=\Pr\{S_t=s^\prime, R_t=r|S_{t-1}=s, A_{t-1}=a\}$
    ...
    state $s^\prime$ and reward $r\in\mathbb{R}$ at time step $t$ when taking action $a$ in
    state $s$ at time step $t-1$. This satisfies the \emph{Markov property}:\index{Markov property}
    $p$ depends only on the current state and not the full history.
\end{itemize}

An \gls{rl} agent's goal is to learn a policy\index{policy} $\pi:\mathcal{S}\times\mathcal{A}\rightarrow [0,1]$
mapping each state $s$ to probabilities of taking each possible action $a$ to maximize
the cumulative reward.

A common way to visualize \glspl{mdp} is via a directed graph: nodes model the state
space $\mathcal{S}$ while edges $(s_i,s_j)$ labeled with an action-reward-probability
tuple $(a,r,p)\in\mathcal{A}\times\mathbb{R}\times[0,1]$ model potential
transitions.
...
While this example is easily solved, \glspl{mdp} with larger state and action spaces present
a much harder problem.

\subsubsection{Partially Observable Markov Decision Processes}\label{sec:rl-pomdp}

In the previous formulation of \glspl{mdp}, the true state of the environment is available
to the agent.
...
In the case of fuzzing, the agent perceives coverage as a partial observation of
the true program state.
...
This situation is modeled through a \emph{\gls{pomdp}}%
\index{Markov decision process!partially observable}
~\cite{ASTROM1965174,kaelbling_planning_1998}, which
extends the original 3-tuple \gls{mdp} $(\mathcal{S},\mathcal{A},p)$ to a 5-tuple $\mathit{POMDP}=(\mathcal{S},\mathcal{A},p,\mathcal{O},o)$, by adding
...
The environment still evolves as an \gls{mdp} over $\mathcal{S}$ according to $p$.
However, the agent must act on observation $O_t$ instead of on the true state $S_t$.
An ordinary \gls{mdp} is the special case $\mathcal{O}=\mathcal{S}$ with $o=\operatorname{id}(\cdot)$.

\glspl{pomdp} introduce the problem of \emph{state aliasing}.\index{state aliasing}
...
A \gls{pomdp} can be made Markovian by treating not the actual observations, but the
entire observation-action history $H_t=O_0,A_0,O_1,A_1,\dots,O_t$ as the state space.\index{observation-action history}
...
This state is called a \emph{belief state}\index{belief state} $b_t(s)=\Pr\{S_t=s|H_t\}$ and suffices
...
the existence of a known model $(p,o)$. Solving the resulting belief \gls{mdp} is also intractable
...
The most important problem any \gls{rl} agent encounters is that of the \emph{exploration-exploitation tradeoff}.\index{exploration--exploitation tradeoff}
...
Balancing the selection of actions that explore versus actions that exploit is a key tradeoff in any \gls{rl} algorithm.

Any \gls{mdp} where $\lvert\mathcal{S}\rvert=1$ and $\lvert\mathcal{A}\rvert=k$ is also known as
a \emph{$k$-armed bandit}\index{k-armed bandit@$k$-armed bandit}, where the $k$-th arm is equal to the $k$-th action $a_k\in\mathcal{A}$.
...
A greedy agent\index{greedy policy} always pulls the arm with the highest current estimate
...
If the agent is unlucky and sees a few bad initial rewards, the sample average $Q_t$ will be skewed and $a_t^*$ will be suboptimal.

\paragraph{$\varepsilon$-greedy}
The simplest remedy is to explore at random.\index{epsilon-greedy@$\varepsilon$-greedy}
...
Directed exploration makes use of the \gls{ucb}\index{upper confidence bound} rule to estimate which arms
could still yield a high reward given how little they have been tried.
...
\Gls{ucb} is generalizable from bandits to full \glspl{mdp} by treating every decision point
of a search tree over the action space as its own bandit.
This gives rise to the \gls{uct} rule\index{upper confidence bound!applied to trees}~\cite{hutchison_bandit_2006} that is used in an
algorithm called \gls{mcts}~\cite{hutchison_efficient_2007}.

\subsection{Solving MDPs}\label{sec:rl-solving}

Methods for solving \glspl{mdp} can be classified along two axes.

\paragraph{Representation of estimates}
Classical algorithms represent estimates via tables.
The size of the tables usually scales with the size of the state and action spaces.
This makes \emph{tabular methods}\index{tabular method} applicable only to small problems with tractable size.
\emph{Function approximation methods}\index{function approximation} exploit regularities in the table data by fitting a parameterized model.
Earlier work that made use of manually engineered features has been succeeded by modern, deep-learning-based approaches~\cite{sutton_reinforcement_2020,simsek_why_nodate,mnihHumanlevelControlDeep2015}.

\paragraph{Model availability}
Some algorithms require that a model of the transition function $p$ be available.\index{model-based method}
Classically, the model is assumed to be specified. In more modern algorithms,
the model is learned by the agent itself through interactions with the underlying \gls{mdp}.
\emph{Model-free methods}\index{model-free method} do not base their estimates on an underlying model, but
derive them directly from the interactions.
\Cref{tab:rl-taxonomy} places the algorithms of this section along both axes.
...
The \emph{return}\index{return} at time step $t$ is the discounted sum of all future rewards,

\begin{equation}\label{eq:return}
  G_t=R_{t+1}+\gamma R_{t+2}+\gamma^2 R_{t+3}+\dots=\sum_{k=0}^{\infty}\gamma^k R_{t+k+1},
\end{equation}

where the \emph{discount factor}\index{discount factor} $\gamma\in[0,1)$ controls how far-sighted the agent is
by exponentially discounting future rewards.

A \emph{value function}\index{value function} measures how good a state is under a given policy.
...
The \emph{Bellman equation}\index{Bellman equation} is a recursive consistency equation that formalizes the
...
An \emph{optimal policy}\index{policy!optimal} $\pi^*$ has the property that no other policy assigns a
...
The recursive \emph{Bellman optimality equation}\index{Bellman equation!optimality} follows:

\begin{equation}\label{eq:bellman-opt}
  v^*(s)=\max_a\sum_{s^\prime,r}p(s^\prime,r|s,a)\left[r+\gamma v^*(s^\prime)\right],
\end{equation}

We can define the same equations over state-action pairs $(s,a)\in\mathcal{S}\times\mathcal{A}$ instead:\index{action-value function}
...
Let $\pi_i$ be given.\index{policy iteration} By repeatedly applying the Bellman equation,
we can compute $v_{\pi_i}$. Once $v_{\pi_i}$ is known, we improve $\pi_i$ by acting greedily
with respect to $v_{\pi_i}$. This then yields a strictly better policy $\pi_{i+1}$.
A fixed point after $n$ steps is then guaranteed to be optimal: $\pi_n=\pi^*$.

\paragraph{Value iteration}
Instead of alternating between deriving $v_{\pi_i}$ and improving $\pi_i$,\index{value iteration} we can directly
...
Most \gls{rl} algorithms make use of this general framework of estimating values and then
using these estimates to improve the current policy.
This framework is called \emph{\gls{gpi}}\index{generalized policy iteration}~\cite{sutton_reinforcement_2020}.
...
\Gls{td} learning\index{temporal-difference learning} removes the need for a model.
...
\Gls{td} methods, like dynamic programming, make use of \emph{bootstrapping}\index{bootstrapping} --- updating estimates based on other estimates.
They do not require full state-space sweeps, but update the value function using the sampled transition data.
A simple \gls{td} algorithm, \gls{td}(0), updates the estimate $V(S_t)$ as follows:
...
where $\alpha\in(0,1]$ is a step size and the \emph{TD error}\index{temporal-difference error} $\delta_t$ measures how much the
sampled outcome deviates from the current estimate.

Monte Carlo methods\index{Monte Carlo method} form the bootstrap-free alternative by simulating a whole
episode, updating toward the full observed return $G_t$ at the end.
The \gls{td}($\lambda$) family interpolates between the two extremes via \emph{eligibility traces}\index{eligibility trace},
with \gls{td}(0) and Monte Carlo as the limit cases $\lambda=0$ and
$\lambda=1$~\cite{sutton_learning_1988}.

\paragraph{SARSA}
\Gls{sarsa}\index{SARSA}~\cite{sutton_reinforcement_2020} is similar to \gls{td} learning, but learns an action-value function $Q(s,a)$ instead of
a value function $V(s)$. The update rule makes use of the action that the agent
actually performed, making \gls{sarsa} an \emph{on-policy}\index{on-policy learning} algorithm since it follows
...
Q-learning\index{Q-learning}~\cite{watkins_q-learning_1992} can be interpreted as an \emph{off-policy}\index{off-policy learning} \gls{sarsa}:
its target maximizes over the successor actions, making the update rule
independent of the actually taken action:
...
The algorithms presented above are tabular: they maintain one entry per state or state-action pair,
requiring a state space small enough to enumerate.
Realistic state spaces are far too large for tables.
Additionally, tabular methods cannot generalize to unseen states.
...
can be learned, for example by (semi-)gradient descent on the \gls{td} error.

Historically, the approximators were linear in hand-crafted features.
Today, they are deep neural networks, giving rise to the field of \emph{deep reinforcement learning}.\index{reinforcement learning!deep}
...
the \gls{dqn}\index{deep Q-network}~\cite{mnih_human-level_2015} and policy gradient
methods\index{policy gradient method} such as
\gls{ppo}\index{proximal policy optimization}~\cite{schulman_proximal_2017}. \Gls{dqn} uses the standard Q-learning update
rule, but represents $Q$ as a function approximator instead, allowing it to
deal with large state and action spaces. \gls{ppo} directly learns a parameterized
...
Model-free deep \gls{rl} algorithms require a large amount of data.
For instance, \gls{dqn} and its successors require on the order of tens to hundreds
...
Many real-world applications therefore require high \emph{sample efficiency}.\index{sample efficiency}

Model-based deep \gls{rl} algorithms allow higher sample efficiency by learning a model of the underlying dynamics
while they interact with the environment.\index{world model}
