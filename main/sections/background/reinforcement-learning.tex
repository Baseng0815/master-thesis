...
Reinforcement learning (RL)\index{reinforcement learning} is an extremely general framework for approaching problems
...
finite Markov decision processes (MDPs).\index{Markov decision process} Formally, a Markov decision process is
...
An RL agent's goal is to learn a policy\index{policy} $\pi:\mathcal{S}\times\mathcal{A}\rightarrow [0,1]$
...
POMDPs introduce the problem of \emph{state aliasing}.\index{state aliasing}
...
The most important problem any RL agent encounters is that of the \emph{exploration-exploitation tradeoff}.\index{exploration--exploitation tradeoff}
...
Balancing the selection of actions that explore versus actions that exploit is a key tradeoff in any RL algorithm.
...
\Gls{ucb} is generalizable from bandits to full MDPs by treating every decision point
...
This framework is called \emph{generalized policy iteration} (GPI)\index{generalized policy iteration}~\cite{sutton_reinforcement_2020}.
...
Temporal-difference (TD) learning\index{temporal-difference learning} removes the need for a model.
...
TD methods, like DP, make use of \emph{bootstrapping}\index{bootstrapping} --- updating estimates based on other estimates.
...
The TD($\lambda$) family interpolates between the two extremes via \emph{eligibility traces}\index{eligibility trace},
with TD(0) and Monte Carlo as the limit cases $\lambda=0$ and
$\lambda=1$~\cite{sutton_learning_1988}.

\paragraph{SARSA}
SARSA\index{SARSA}~\cite{sutton_reinforcement_2020} is similar to TD learning, but learns an action-value function $Q(s,a)$ instead of
a value function $V(s)$. The update rule makes use of the action that the agent
actually performed, making SARSA an \emph{on-policy}\index{on-policy learning} algorithm since it follows
...
Q-learning\index{Q-learning}~\cite{watkins_q-learning_1992} can be interpreted as an \emph{off-policy}\index{off-policy learning} SARSA:
...
the deep Q-network (DQN)\index{deep Q-network}~\cite{mnih_human-level_2015} and policy gradient
methods\index{policy gradient method} such as proximal policy optimization
(PPO)\index{proximal policy optimization}~\cite{schulman_proximal_2017}. DQN uses the standard Q-learning update
