...
Since execution throughput is usually proportional to the effectiveness of a fuzzer,
this is an important advantage~\cite{fioraldi_afl_nodate,manes_art_2019}.
It also does not require an input model: an iteratively refined seed corpus can
...
\textcite{bohme_fuzzing_2020} has shown that although linearly increasing the fuzzing budget
produces a linear reduction in time required to find a given set of bugs, actually
discovering new code requires an exponentially increasing fuzzing budget.
AFLFast attributes this plateau on the budget being spent on high-frequency
...
techniques like the ones used by AFL.\todo{what to cite?}.
\textcite{she_neuzz_2019} show that by learning an approximation to a PUT's branching
behavior, fuzzing performance can be improved.
\textcite{she_mtfuzz_2020} use taint analysis to discover more edges than  baseline AFL.
...
execution~\cite{stephens_driller_2016}, by learning an input-to-state
correspondence~\cite{aschermann_redqueen_2019}, or by taint
tracking~\cite{gan_greyone_nodate}.

Coverage itself might also be an insufficient objective to optimize for. While
it has been empirically proven to be effective for general PUT's, in some use
...
AFL++'s \textsc{ChooseNext} and \textsc{AssignEnergy} are adaptive, but depend on a few hand-crated metrics,
