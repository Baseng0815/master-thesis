...
Its goal is to find inputs to a \gls{sut} that trigger a certain
...
\begin{enumerate}
  \item \emph{\Gls{sut}:}\index{system under test}
  ...
  \item \emph{Security Policy:}\index{security policy}
    A security policy defines which observable behaviors of the \gls{sut} are acceptable.
  ...
  \item \emph{Bug Oracle:}\index{bug oracle}
    A bug oracle is a program that decides whether a given input to the \gls{sut} violates
    a given security policy. A widely used bug oracle might deem an execution
    a violation if the \gls{sut} crashes or hangs.

  \item \emph{Fuzzing:}\index{fuzzing}
    Fuzzing denotes the process of feeding the \gls{sut} with inputs sampled from an input space.
  ...
  \item \emph{Fuzzer:}\index{fuzzer}
    A fuzzer is a program that performs fuzz testing on a \gls{sut}.

  \item \emph{Fuzz Campaign:}\index{fuzz campaign}
    A fuzz campaign is a specific execution of a fuzzer on a \gls{sut} with a specific
    security policy. Its goal is to find bugs in the \gls{sut} that violate this policy.

  \item \emph{Test Case:}\index{test case}
    A test case denotes a concrete input that the fuzzer feeds to the \gls{sut} during
  ...
  \item \emph{Seed:}\index{seed}
    A seed is a commonly well-structured input to the \gls{sut} that the fuzzer uses
...
information from the execution of the \gls{sut} available to them~\cite{liang_fuzzing_2018}.

\subsubsection{Black-box fuzzing}

Black-box fuzzers\index{fuzzing!black-box} have no access to the internal behavior of a \gls{sut} during execution.
...
The disadvantage is the lack of information about \gls{sut} behavior.
...
the internal logic of the \gls{sut} as well as usually any original source code or
...
to large \glspl{sut} due to the path explosion problem.\index{path explosion}
...
of the internal behavior of the \gls{sut} might still be possible, \textit{greybox}
...
better judge the behavior of the \gls{sut} for a given input and direct its exploration efforts towards promising regions.
...
the \gls{sut}, often represented through a grammar specification.
...
TestMiner~\cite{toffola_saying_2017} searches for literals inside the \gls{sut} and derives suitable inputs.
PULSAR~\cite{thuraisingham_pulsar_2015} infers a network protocol from a set of captured network packets generated by the \gls{sut}.

Encoder-based models interpret the \gls{sut} as a decoder for inputs generated by an existing encoder.
...
shallowest layers of a \gls{sut}~\cite{manes_art_2019,godefroid_sage_2012}.
In general, a \gls{sut} contains checks that reject invalid inputs, making it
...
to the \gls{sut} like valid network packets or files.
...
This work will focus primarily on \gls{cgf}\index{fuzzing!coverage-guided greybox} in its greybox variant.
It is the dominant design among modern general-purpose fuzzers, popularized by
\gls{afl}~\cite{zalewski2014afl} and its successor AFL++~\cite{fioraldi_afl_nodate}.
...
If the fuzzer deems the execution of the \gls{sut} under $t^\prime$ to be interesting, $t^\prime$
...
\Cref{alg:cgf} states the \gls{cgf} loop.
...
mutated input or not based on \gls{sut} behavior (\textsc{IsInteresting}).
...
This coverage is not known to the fuzzer unless it executes the \gls{sut} with a mutated seed,
...
Collecting coverage information requires instrumenting the \gls{sut}.
...
\begin{enumerate}
  \item \emph{Compile-time instrumentation}\index{instrumentation!compile-time}
    inserts instrumentation during compile time of the \gls{sut}.
...
\gls{afl}'s mutation pipeline has two stages~\cite{fioraldi_afl_nodate}.
...
\gls{afl} implements \textsc{IsInteresting} through a hybrid metric combining edge coverage with
...
implicitly generate inputs that adhere to the specification and grammar of the \gls{sut}.
This makes \gls{cgf} applicable to a wide range of targets~\cite{manes_art_2019}.
...
\gls{cgf} is also blind to any input-coverage correspondence~\cite{manes_art_2019}, therefore
...
techniques like the ones used by \gls{afl}.
\textcite{she_neuzz_2019,she_mtfuzz_2020} show that learning an approximation to a \gls{sut}'s branching
behavior improves fuzzing performance.
\textcite{gan_greyone_nodate} use taint analysis to discover more edges than baseline \gls{afl}.
Further approaches are surveyed by~\textcite{wang_systematic_2020} and~\textcite{qiu_survey_2025}.

Magic bytes and checksums present a major barrier to the success of \gls{cgf} since
they are highly unlikely to be found by random mutation (\cref{sec:fuzzing-input-generation}).
Overcoming these checks can be done by augmenting \gls{cgf} with symbolic
...
it has been empirically proven to be effective for general \glspl{sut}, in some use
cases like embedded systems, corruption of a \gls{sut}'s state is not related to its
coverage~\cite{muench_what_2018} or might require additional sanitization
efforts~\cite{zhang_debloating_nodate}.

Coverage is also a lossy representation that only approximates the \gls{sut}'s state
...
\gls{cgf} models the \gls{sut} as a pure function mapping an input to a coverage.
A new input requires the \gls{sut} to be reset to its initial state.
For stateful \glspl{sut}, the coverage depends on the current input as well as the
history of previous inputs.
Stateful targets\index{fuzzing!stateful} are especially prevalent in \gls{iot} applications and present a
...
network protocols like QUIC~\cite{ang_quic-fuzz_2025} and \gls{opcua},
...
The state of the \gls{sut} as seen from the execution engine.\index{state!program} It is made up
...
The state inside the protocol as given by a protocol specification.\index{state!protocol} For a \gls{sut} initiating
...
therefore cannot deal natively with highly stateful \glspl{sut}.
...
and works by combining traditional \gls{cgf} with automatic state inference to guide exploration
