...
A prominent example is \emph{Driller}, which alternates pure greybox fuzzing with
...
TestMiner~\cite{toffola_saying_2017} searches for literals inside the PUT and derives suitable inputs.
...
\begin{enumerate}
  ...
  \item \emph{Dictionary-based mutation:}
    Dictionary-based mutation substitutes values with other values of high semantic
...
\emph{exploration versus exploitation conflict}: time can be spent either on gathering more
...
Another problem lies in the existence of magic bytes or checksums, which are hard to satisfy by random mutations.
...
To decide whether a new coverage map is interesting, the fuzzer compares it against
...
\textcite{she_neuzz_2019,she_mtfuzz_2020} show that learning an approximation to a PUT's branching
