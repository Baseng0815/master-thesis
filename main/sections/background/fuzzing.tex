...
\begin{enumerate}
  \item \emph{Program Under Test (PUT):}\index{program under test}
...
Black-box fuzzers\index{fuzzing!black-box} have no access to the internal behavior of a PUT during execution.
...
to large PUTs due to the path explosion problem.\index{path explosion}
...
\emph{concolic} execution\index{concolic execution} (a combination of concrete and symbolic execution) to overcome hard branch
...
The state of the PUT as seen from the execution engine.\index{state!program} It is made up
of memory and register file state.

\paragraph{Protocol state}
The state inside the protocol as given by a protocol specification.\index{state!protocol} For a PUT initiating
