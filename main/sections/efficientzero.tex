...
$s^{i+1}=h(o^{i+1};\theta)$, we can \emph{imagine} forwarding the latent.
...
calculated from the predicted successor latent $\hat{s}^k=g(s^{k-1},a_{t+k};\theta)$
...
training signal.
The idea is that the imagined next state $\hat{s}^{k}=g(s^{k-1},a^{k};\theta)$
and the encoded real next state $s^{k}=h(o_{t+k};\theta)$ should be similar.
...
\begin{equation}\label{eq:ez-consistency}
  ...
  = -\operatorname{cossim}\!\Big(
      \operatorname{sg}\big(\projnet(s^{k};\theta)\big),\;
      \prednet\big(\projnet(\hat{s}^{k};\theta);\theta\big)
    \Big),
\end{equation}
