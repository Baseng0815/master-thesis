...
(MCTS)\index{Monte-Carlo tree search}~\cite{hutchison_bandit_2006,hutchison_efficient_2007} is a tree search
...
to align the current policy with the improved MCTS policy.\index{training target!policy}

\paragraph{Value target}
The value head is trained toward an $n$-step return of actual rewards,
bootstrapped with the root value that MCTS produced:\index{training target!value}
