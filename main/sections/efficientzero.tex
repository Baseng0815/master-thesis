...
\chapter{The EfficientZero Algorithm}\label{chap:efficientzero}

EfficientZero\index{EfficientZero}~\cite{ye_mastering_2021} is a model-based deep reinforcement
...
benchmark\index{Atari 100k}~\cite{ye_mastering_2021}.

\section{From AlphaGo to EfficientZero}\label{sec:ez-lineage}

AlphaGo\index{AlphaGo}~\cite{silver_mastering_2016} was published in 2016 and was the first program to defeat a
...
AlphaGo Zero\index{AlphaGo!Zero}~\cite{silver_mastering_2017-1} removes the dependency on human
data.
It learns exclusively from self-play, using a single network with a policy and a value head.
The training loop is an instance of \gls{gpi} (\cref{sec:rl-dp}): \gls{mcts} improves
the policy and the improved policy is used to generate new games to train the
networks to predict the output of \gls{mcts}.
AlphaGo Zero defeated the published AlphaGo by 100 games to 0.

AlphaZero\index{AlphaZero}~\cite{silver_general_2018} rids itself entirely of the Go-specific dependencies.
...
AlphaZero still requires a perfect simulator for \gls{mcts}.

MuZero\index{MuZero}~\cite{schrittwieser_mastering_2020} removes the dependency on the simulator.
...
MuZero matched AlphaZero's performance in board games while
...
The representation network\index{network!representation} $h:\mathcal{O}\to\mathcal{S}$ maps observations $o_t$
to a latent representation $s_t$.\index{latent state}
...
The dynamics network\index{network!dynamics} $g:\mathcal{S}\times\mathcal{A}\to\mathcal{S}$ maps a
current latent $s^i$ and an action $a^i$ to the next latent
$s^{i+1}=g(s^i,a^i;\theta)$.
A single forwarding step is known as a \emph{rollout} step.\index{rollout step}
...
The value network\index{network!value} $v:\mathcal{S}\to\mathbb{R}$ estimates the expected discounted
return (\cref{eq:value-function}) of a state $s_t$.

\paragraph{Value-prefix network $\vpnet$}
The value prefix\index{network!value-prefix} $\vpnet:\mathcal{S}\times\mathcal{A}^k\to\mathbb{R}$ estimates
...
\gls{mcts} to guide its search towards high-valued actions and states.

\paragraph{Policy network $p$}
The policy network\index{network!policy} $p:\mathcal{S}\to\Delta(\mathcal{A})$ maps a state $s_t$ to a
probability distribution over actions, whose component for action $a$ is written
$p_a(s)$.
It models a learned policy $\pi(a|s)$ and acts as a search prior for \gls{mcts}.

\section{Monte-Carlo Tree Search}\label{sec:ez-mcts}

\Gls{mcts}\index{Monte-Carlo tree search}~\cite{hutchison_bandit_2006,hutchison_efficient_2007} is a tree search
algorithm. It makes use of the \gls{uct} rule that derives from the \gls{ucb} rule (\cref{eq:ucb}) introduced in
...
The use of a prior entails a slight modification of the \gls{uct} rule, called pUCT (prior UCT).\index{upper confidence bound!prior (pUCT)}
...
\gls{mcts} is applied in each state and produces a root visit count distribution as well
...
\gls{mcts} itself is a general framework that can be adapted to many situations.
It can be interpreted as a policy improvement operator:\index{policy improvement operator} given a policy prior
...
The \gls{mcts} variant used in EfficientZero consists of three phases that are
repeated a fixed number of times: \emph{selection}\index{Monte-Carlo tree search!selection},
\emph{expansion}\index{Monte-Carlo tree search!expansion}, and
\emph{backup}\index{Monte-Carlo tree search!backup}.
\gls{mcts} stores the current state of its search tree in nodes, where
...
node is reached~(\cref{fig:ez-mcts-selection}).
...
To improve exploration, the root priors are perturbed with Dirichlet noise\index{Dirichlet noise}
...
it is expanded~(\cref{fig:ez-mcts-expansion}): the previous latent state is
unrolled one step to get the new latent $s^{k+1}=g(s^k,a^k;\theta)$ and the value
...
$s^0, a^1, s^1, \dots, a^l, s^l$~(\cref{fig:ez-mcts-backup}).
...
New data is generated by interacting with the environment.\index{worker!data}
At every time step $t$, a worker encodes the latest observation into
$s_t = h(o_t;\theta)$ and runs \gls{mcts} from this root.
The resulting root visit count distribution\index{visit count distribution} is then sampled using a temperature
$T$\index{temperature} and the action passed to the environment:
...
The resulting observation-reward tuples $(o,r)$ are stored in a replay buffer,\index{replay buffer}
...
prepare training batches.\index{worker!rollout}
Each trajectory position inside the replay buffer has a sampling priority associated with it\index{prioritized experience replay}~\cite{schaul_prioritized_2016}.
This priority is proportional to the loss of the value network $v$ evaluated at this position.
Prioritizing positions with a high loss accelerates learning.

\paragraph{Reanalyze workers}

Prepared batches are then processed by reanalyze workers.\index{worker!reanalyze}
\emph{MuZero Reanalyze}\index{MuZero!Reanalyze} is an extension to the basic MuZero algorithm that improves the freshness of
data by re-running \gls{mcts} on old data with updated model weights, contributing to data reuse and sample efficiency.
Reanalyzed batches are finally passed to the \gls{gpu} workers where the model is trained.

This architecture is an instance of \gls{gpi}:
\gls{mcts} serves as the policy improvement operator~\cite{grill_monte-carlo_2020}, while learning from stored experience
...
unrolled\index{unroll} for $K$ steps with the actions that were actually taken,
...
as \gls{mcts} root visit distribution and root value target.

\paragraph{Policy target}
The policy head is trained toward the \gls{mcts} root visit distribution $\pi_{t+k}$
to align the current policy with the improved \gls{mcts} policy.\index{training target!policy}

\paragraph{Value target}
The value head is trained toward an $n$-step return of actual rewards,
bootstrapped with the root value that \gls{mcts} produced:\index{training target!value}

\begin{equation}\label{eq:ez-value-target}
  z_t=\sum_{i=0}^{n-1}\gamma^{i}\,r_{t+i+1}+\gamma^{n}\,v^{\text{MCTS}}_{t+n},
\end{equation}

where $u$ are the actual rewards observed by the data workers and
$v^{\text{MCTS}}_{t+n}$ is the \gls{mcts} root value.
...
\Cref{app:loss} gives the layout of a sampled batch, the masks that decide which positions each
term supervises, and the exact form the four terms take in our implementation.

\section{EfficientZero's Improvements}\label{sec:ez-improvements}

\Textcite{ye_mastering_2021} identify three weaknesses of MuZero and address
each of them with one change:
...
EfficientZero adds a SimSiam-inspired\index{SimSiam}~\cite{chen_exploring_2020}
training signal.\index{consistency loss}
...
uses BYOL~\cite{grill_bootstrap_2020}, a method relying on a momentum encoder,
for the same purpose in a model-free agent.
...
EfficientZero sidesteps this issue by directly predicting the \emph{value prefix}\index{value prefix} $x_{t,k}$,
a sum of discounted rewards over a given sequence of actions:

\begin{equation}\label{eq:ez-value-prefix}
  x_{t,k}=\sum_{i=0}^{k-1}\gamma^{i}\,r_{t+i+1}.
\end{equation}
...
Off-policy bias of multi-step targets is a classical problem.\index{off-policy correction}
...
\begin{equation}\label{eq:ez-corrected-target}
  z_t=\sum_{i=0}^{l-1}\gamma^{i}\,r_{t+i+1}+\gamma^{l}\,v^{\text{MCTS}}_{t+l}.
\end{equation}
