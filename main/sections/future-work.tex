...
% \cref{sec:discussion-interpretation} or \cref{sec:discussion-threats} rather than
% listed generically. The closing paragraph of \cref{sec:discussion-interpretation}
% already splits them into implementation limits (remedied by engineering) and
% approach limits (open problems); only the latter belong here. Candidates:
%
...
%   - A corpus of more than one seed: every run keeps a single seed
%     (\cref{sec:impl-corpus}), which makes selection and eviction degenerate. Raising
%     the corpus size needs per-slot observation rows for both heads to rank slots with,
%     and those rows must be slot-identity-preserving.
...
%   - Reliability: every corpus target ends below its own structural-rate peak
%     (\cref{fig:structural-decay}), so a stopping rule --- or training that does not
%     give ground back --- is worth more than making converged runs better.
