...
but also on a history of inputs.
Examples of such targets include highly relevant software systems such as
industrial control systems, and embedded and \gls{iot}
...
Some emulators also allow the fuzzer to take a lightweight snapshot of a program's
...
\Gls{rl} is a machine learning paradigm that aims to solve these
kinds of problems.
While model-free \gls{rl} methods have been applied to fuzzing before~\cite{bottinger_deep_2018,binosi_rainfuzz_2023,wang_reinforcement_2021,wang_syzvegas_2021,borcherding_bandits_2023},
the use of model-based \gls{rl} promises higher sample efficiency, requiring fewer
executions of a program to achieve a given goal.

EfficientZero~\cite{ye_mastering_2021} is a model-based \gls{rl} method that learns a
...
The main question this thesis addresses is whether model-based \gls{rl} can be
...
\begin{enumerate}[label=\textbf{RQ\arabic*.}, ref=RQ\arabic*, leftmargin=*]
  ...
  \item\label{rq:action-space}
    How should the fuzzing action space be modeled?
...
\begin{enumerate}
  \item \emph{Two \gls{mdp} formulations of emulator-based greybox fuzzing} that allow
    \gls{rl} algorithms to drive fuzzing of various target executables.
  ...
  \item \emph{An empirical evaluation} including a detailed ablation study
    on a simple \gls{json} parser as well as fuzzing runs for various programs.
\end{enumerate}
...
\cref{chap:related-work} surveys prior applications of \gls{rl} to fuzzing and the
difficulties they report.

\cref{chap:fuzzing-mdp} models emulator-based fuzzing as a Markov decision
process and states how we model states, actions and rewards.
\cref{chap:implementation} describes the resulting system, including
the execution environment, EfficientZero learner and MOGI execution environment.

\cref{chap:targets} introduces the fuzzing targets, and \cref{chap:methodology}
the experimental methodology.
\cref{chap:results} reports the results for both formulations, and
\cref{chap:discussion} interprets them: what the approach demonstrates, where
it succeeds and what its limits are.
\cref{chap:conclusion} answers the research questions and
