...
\cref{chap:platform} presents MOGI, the emulation platform this thesis builds
on, and the coverage and snapshot primitives it provides.
\cref{chap:fuzzing-mdp} models emulator-based fuzzing as a Markov decision
process.
\cref{chap:input-actions,chap:corpus-actions} introduce the two different ways
we model the action space.
\cref{chap:implementation} describes the resulting system.
