...
At \num{1664128} guest executions the cJSON baseline reaches a median of \num{645} covered blocks
...
control distribution --- separates them by a factor of roughly \num{71}, \SI{76.66}{\percent}
against \SI{1.08}{\percent}, on every paired seed~(\cref{tab:deep-seed}).
...
training~(\cref{fig:collapse-entropy}).
Reward starvation does not explain that decay.
Novelty is scored against each episode's own corpus, which is rebuilt at every reset, so a block
found once pays again in the next episode and the share of replay transitions carrying a reward
rises over training rather than falling --- on cJSON from \num{0.012} to
\num{0.147}~(\cref{fig:buffer-reward-density}).
The agent gives up structural ground while its reward signal is growing denser.
