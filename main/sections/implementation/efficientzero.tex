...
EfficientZero is a RL algorithm and therefore formally interacts with a MDP.
...
In practice, we implement this by keeping a pool of $n$ active environments and step
them in parallel while resetting any environments that have terminated.

Another bottleneck are host-device transfers between CPU and GPU.
...
Because it must be efficiently sampleable for training, we use a struct-of-array representation
over ring buffers.
Since episodes may have different lengths, we additionally keep metadata such
as the offset of an episode into the data arrays as well as the episode length
...
For example, a single cjson (\todo{cref to cjson}) environment step carries with it

If the replay buffer is full, the oldest trajectory is evicted. Sampling
happens per step rather than per in accordance to \gls{per}. Freshly inserted
episodes receive the largest priority observed so far, so that new data is
sampled at least once. Priorities are updated by the reanalyze workers once the
value error of a position is known.
...
Wrapping all GPU work in a explicit GPU worker allows to better control queue
contention and data synchronization.
It also allows adding a second GPU by spawning a second worker in the future.

\input{figures/impl-ez-workers}
...
This allows to build things like a visualization dashboard or persistence layer
