...
using the Burn deep learning framework.\index{Burn}
...
EfficientZero is an \gls{rl} algorithm and therefore formally interacts with an \gls{mdp}.
Making use of the rich type system of Rust, we model the \gls{mdp} via an \code{Environment} trait
with associated \code{Action} and \code{Observation} types.\index{environment interface}
...
\Cref{lst:ez-abstraction} shows the resulting interface.
...
is the quick alternation between model inference on the \gls{gpu} and using said inference
results for \gls{cpu} work.
...
The solution lies in batching as many \gls{gpu} calls as possible.
...
Another bottleneck is host-device transfers between \gls{cpu} and \gls{gpu}.
...
Importantly, a sampled batch is assembled on the host and contains no \gls{gpu} data.
...
Since we assume the \gls{gpu} to always be the bottleneck for training as well as data generation,
...
All \gls{gpu} work is done by \gls{gpu} workers.\index{worker!GPU}
...
Wrapping all \gls{gpu} work in an explicit \gls{gpu} worker gives better control over queue
...
includes a way to add various kinds of hooks\index{hook} to different parts of the learning process.
