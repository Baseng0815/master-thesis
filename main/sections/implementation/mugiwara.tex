...
saturates at level 8, so hit counts of 128 and above are no longer distinguished.
The block hit score is the sum of the levels over all buckets.
For a maximum number of 4096 blocks, the highest possible score is therefore
$4096\times 8=32768$.
Both terms of the reward introduced in~\cref{sec:mdp-rewards} are computed from
this score.
The breadth term compares the levels of the current execution against those of
the union coverage, and the depth term compares its score against the highest
score of any single execution of the episode so far.
Accumulation of coverage into the union happens by taking the per-bucket maximum,
which is what makes both terms telescope.
