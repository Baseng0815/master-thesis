...
It does this by implementing the \code{Environment} trait for each \gls{sut}.
...
modulo 32.
An episode is fixed at 64 mutations, which equals 128 \gls{mdp} steps and 64 guest
...
consists of pushing a single input, usually a byte, into the input stream of the \gls{sut}
...
During each step, the \gls{sut} is re-executed with a given seed.
This is done by resetting the environment and block hit trackers, pushing the
seed data, waking and running the \gls{sut}, and collecting any observations and is
...
Actual \gls{gpu} compute is also a bottleneck, but only slows down parallel runs and does
