...
It does this by implementing the \code{Environment} trait for each PUT.
...
The hook receives the block address and passes it to a \code{BlockHitTracker},
which hashes it, folds the hash into one of 4096 buckets and increments that bucket's counter,
similar to AFL++.

The bucket count trades memory against collisions and is target-dependant.
...
For example, cjson\todo{cref to cjson} hits 495 unique blocks on startup, so a bucket
...
Block hits are then converted to a reward.
First, a \code{Coverage} converts raw block hit counts into exponentially
spaced hit-count levels.
Hit-count level $n$ in this case spans $2^{n-1}$ to $2^n-1$ raw block hits and
saturates at level 8, so the maximum number of raw block hit counts that can be registered
is $2^8=256$.
The block hit score is the sum of the levels over all buckets.
For a maximum number of 4096 blocks, the highest possible reward is therefore
$4096\times 8=32768$.
The stateful reward $r_\beta$ introduced in~\cref{sec:mdp-rewards} can be
implemented by deriving the reward from the cumulative block hit tracker in the
way described above, while the stateless reward $r_\alpha$ can be implemented
by the \emph{marginal coverage} i.e., the difference between the coverage of
the previous execution and the current execution.
Accumulation of coverage happens by taking the per-bucket maximum.

\subsection{The Seed Corpus}\label{sec:impl-corpus}

The \code{corpus} module implements a target-agnostic seed corpus as described in \cref{sec:mdp-actions}.
...
of all seeds seen so far.
This means even seeds that have been discarded before will have their coverage included in
the union.
While this might seem wrong at first, this approach is important to guarantee that
the marginal coverage contains a proper novelty signal, and that the learner is not
just re-deriving already tested seeds.
...
We therefore split a single environment step artifically into two steps, where the
...
Each program is run once until it yields and then a snapshot is created.
...
consists of pushing a single input, usually a byte, into the input stream of the PUT
...
During each step, the PUT is re-executed with a given seed.
This is done by resetting the environment and block hit trackers, pushing the
seed data, waking and running the PUT, and collecting any observations and is
...
A probe in this context is a set of hooks (~\cref{hooks} that collect different
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}llX@{}}
    ...
    \code{LossProbe}
      ...
      & the five loss terms of one gradient step \\
    \code{ReturnProbe}
...
An \code{Evaluation} is the full description of a single run.
It describes which target is fuzzed, which seed to use, and which settings to apply.
A campaign is an ordered queue of evaluations and is drained by a loop while allowing
...
This can either be an insufficient amount of RAM to hold multiple replay buffers with
the desired capacity, or an insufficient amount of VRAM to store all Tensors at once.
