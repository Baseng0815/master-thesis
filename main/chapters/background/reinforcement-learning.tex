...
An \emph{optimal policy} $\pi^*$ has the property that no non-optimal policy has
a higher valued state: $\forall \pi,\pi\text{ non-optimal}:\forall s\in\mathcal{S}.v_{\pi^*}(s)\geq v_\pi(s)$.
...
\begin{align}
  ...
  q^*(s,a)&=\sum_{s^\prime,r}p(s^\prime,r|s,a)\left[r+\gamma\max_{a^\prime}q_*(s^\prime,a^\prime)\right]\label{eq:bellman-opt-q}
\end{align}
