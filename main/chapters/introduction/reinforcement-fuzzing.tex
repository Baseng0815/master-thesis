...
\section{Reinforcement Learning for Fuzzing}\label{sec:reinforcement-fuzzing}

% Drafting outline: bullet points to be fleshed out into prose.

\subsection{Fuzzing as a Reinforcement Learning Problem}\label{sec:rlfuzz-framing}
...
\subsection{Prior Work}\label{sec:rlfuzz-prior-work}
...
\subsection{Recurring Difficulties}\label{sec:rlfuzz-difficulties}
...
\begin{itemize}
  ...
  \item Honest framing for the thesis: these difficulties define the burden of proof for the
    approach evaluated in \cref{chap:analysis}.
\end{itemize}

\subsection{The Gap Addressed by This Thesis}\label{sec:rlfuzz-gap}
  ...
  \item Unexplored combination --- the subject of this thesis:
    \begin{itemize}
      ...
      \item acting \emph{directly} at the protocol level against a \emph{stateful} target ---
        ...
        (\cref{chap:efficientzero-fuzzing}),
      \item on targets rehosted in an emulator (MOGI, \cref{chap:mogi-setup}), whose snapshots
    ...
    \end{itemize}
  \item Research questions to state explicitly here (evaluated in \cref{chap:analysis}):
    \begin{itemize}
      ...
      \item Where does planning with a learned model help or fail compared to classic greybox
        baselines (\cref{sec:analysis-advantages,sec:analysis-disadvantages})?
    \end{itemize}
\end{itemize}
